...
int main()
{
    ...
    auto update_storage = std::make_shared<Storage<long, FSM>>();
    ...
    bot->getEvents().onCallbackQuery(handlers::show_data(bot, id_storage));
    bot->getEvents().onCallbackQuery(handlers::update_data(bot, update_storage));
    bot->getEvents().onAnyMessage(handlers::update_data_message(bot, update_storage));
    bot->getEvents().onCallbackQuery(handlers::start_update_data(bot, update_storage, id_storage));
    bot->getEvents().onCallbackQuery(handlers::delete_data(bot, id_storage));
